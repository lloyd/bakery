...
# NO 3rd party binaries, NO third party source in tree.  The bakery is *recipes only*.
# in general we build *static libraries*.  The thought is that because we're taking a size hit by shunning libraries present on the system, we can leverage the static linker to minimize that cost.
